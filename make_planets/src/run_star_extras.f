...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      contains
      ! added by SH -- compute total energy of envelope
      subroutine eval_Energies(s,Etot,Eenv,Eenv_int)
         use const_def
         type (star_info), pointer :: s
         integer                   :: k
         real(dp)                  :: Menc,U
         real(dp)                  :: Etot,Eenv,Eenv_int
         real(dp)                  :: e_grav,e_therm
         real(dp)                  :: dMenv
         ...
         do k=(s%nz),1,-1
             e_therm = s% dm(k) * exp_cr(s%lnE(k))
             e_grav  = -1 *s% dm(k) * (s%cgrav(k) * s%m_grav(k) / s%r(k) )
             Eenv_int = Eenv_int + e_therm
             Eenv = Eenv + e_therm + e_grav
         end do
         Etot = Eenv + s%T(s%nz) * cv_core * s%M_center
         return
      end subroutine eval_Energies
      
      ! added by SH -- compute total energy of envelope
      subroutine eval_Etot(s,Etot)
         use const_def
         type (star_info), pointer :: s
         integer                   :: k
         real(dp)                  :: Menc,U
         ...
         val = 0
         do k=(s%nz),1,-1
             !val = val + s% dm(k)*(exp_cr(s% lnE(k))) 
             val = val + s% dm(k)*( &
                exp_cr(s% lnE(k)) - &
                s% cgrav(k)*s% m_grav(k)/s% r(k) )
         end do
         Etot = val
         return
      end subroutine eval_Etot

      ! added by SH -- compute total internal energy of envelope
      subroutine eval_Eint(s,Eint)
         use const_def
         type (star_info), pointer :: s
         integer                   :: k
         real(dp)                  :: Menc,U
	 ...
	 ! INITALIZE
      ...
      subroutine planet_evap(id, Lsurf, Msurf, Rsurf, Tsurf, w, ierr)
       ...
       evap_R=0.0
       evap_Mdot=0.0
       write(*,*) "attempting photoevaporation table read-in"

       do n=1,6
        !write(evap_table,'(a,I1.1,a)') "/projects/p20783/sjh890/01_MESA_Projects/Owen_Rates/Mdot",n,".out"      
       ...
       ! read in new table
       !evap_table='/projects/p20783/sjh890/01_MESA_Projects/Owen_Rates/Mdot_out_Mar15.out'
...
!          endif
      ...
      integer function how_many_extra_history_columns(s, id, id_extra)
         type (star_info), pointer :: s
         integer, intent(in) :: id, id_extra
         ! Added by SH to track internal luminosity
      ...
      subroutine data_for_extra_history_columns(s, id, id_extra, n, names, vals, ierr)
         ...
         real(dp) :: dLdr_ahead,dLdr_behind,r_ahead,r_behind,d2Ldr2,d2Ldr2_max
         integer  :: k,kmax
         real(dp) :: Etot,Eint,Eenv,Lint
              
        
         ! Added by SH
         ! finds the internal luminosity by searching for the cell with the
         ! maximum luminosity second derivative. This occurs where planets
         ! transition from internal luminosity to re-radiated stellar radiation

         d2Ldr2_max = 0.0
         kmax = -1

         ! Reverse loop structure works from innermost zone outward
         ! Skip the first few zones to avoid weird derivative behaviour there
         do k = (s%nz-4),2,-1
                dLdr_ahead = (s%L(k+1) - s%L(k))/(s%r(k+1) - s%r(k))
                dLdr_behind = (s%L(k) - s%L(k-1))/(s%r(k) - s%r(k-1))
                r_ahead =  0.5 * (s%r(k+1) + s%r(k))
                r_behind = 0.5 * (s%r(k) + s%r(k-1))
                d2Ldr2 = abs((dLdr_ahead-dLdr_behind)/(r_ahead-r_behind))
                if (d2Ldr2 .gt. d2Ldr2_max) then
                        d2Ldr2_max = d2Ldr2
                        kmax = k
                endif
         enddo
         call eval_Energies(s,Etot,Eenv,Eint)
         Lint = s%L(kmax+2)
         names(1) = "internal_luminosity"
         names(2) = "kh_timescale_fixed"
         names(3) = "internal_energy"
         names(4) = "envelope_energy"
         names(5) = "total_energy"
         ! kmax+2 get luminosity a little deeper in for safety 
         vals(1) = Lint / lsol
         vals(2) = abs(Etot) / Lint / secyer
         vals(3) = Eint
         vals(4) = Eenv
         vals(5) = Etot
         ierr = 0
